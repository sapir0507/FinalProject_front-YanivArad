...
function UsersTableComp({productID, customersSelect, purchasesSelect}) {
    ...
    useEffect(()=>{
        let tempRows = [];
        if(purchasesSelect){
            const productPurchases = purchasesSelect.filter(pur=> {
              return pur.ProductId === productID + ''
            });
            if(productPurchases)
            productPurchases.forEach(item => {
            const cus = customersSelect.find(cus=> {
     ...
     <TableContainer component={Paper}>
      ...
      <TableHead>
          <TableRow>
            ...
            <StyledTableCell align="center">Add New Product To Customer</StyledTableCell>
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row.name} sx={{color:'white', 
            '&':{
                'a':{
                    color: 'white'
                }
